  ...
  describe('trackAgentButtonClick', () => {
    ...
    it('Should respond with a valid response when required and optional parameters are provided', (done) => {
      ...
      tracker.on('success', (responseParams) => {
        ...
        expect(fetchSpy).to.have.been.called;
        expect(requestParams).to.have.property('section').to.equal('Products');
        expect(requestParams).to.have.property('position_on_page').to.equal(optionalParameters.positionOnPage);
        expect(requestParams).to.have.property('page_type').to.equal(optionalParameters.pageType);
        expect(requestParams).to.have.property('instance_id').to.equal(optionalParameters.instanceId);
      ...
      expect(tracker.trackAgentButtonClick({ ...requiredParameters, ...optionalParameters })).to.equal(true);
    });
    ...
    if (!skipNetworkTimeoutTests) {
      it('Should be rejected when network request timeout is provided and reached', (done) => {
        ...
        expect(tracker.trackAgentButtonClick(requiredParameters, { timeout: 10 })).to.equal(true);
      });
